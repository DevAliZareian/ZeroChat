      ...
      <Box w="300px" h="100vh" overflowY="auto" borderRight={`1px solid ${currentColors.layout.border}`} p={4} bg={currentColors.layout.surface}>
        ...
        <VStack spacing={2} mb={6} position="relative">
          <Box position="relative">
            <Avatar size="xl" name={user?.email} src="/avatars/user.png" border={`1px solid ${currentColors.layout.border}`} />
        ...
        <VStack spacing={3} align="stretch">
          {conversations.map((conv) => (
            <Box key={conv.id} onClick={() => navigate(`/app/chat/${conv.id}`)} p={2} borderRadius="md" cursor="pointer">
              <HStack spacing={3}>
                ...
                <Text fontSize="xs">{conv.last_message?.timestamp}</Text>
              </HStack>
            </Box>
          ))}
        </VStack>
